...
##################################################################################### 
#~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o
## Method 2 (parallel computing with snowfall, rstream for RNG)
#~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o
...
t2a <- system.time({
  ...
  sfLibrary("sna", character.only = TRUE)
  sfLibrary("ergm", character.only = TRUE)
  sfLibrary("RDS", character.only = TRUE)
  
  # replace the for loop with sfSapply: snowfall's equivalent of sapply
  est2a <- sfSapply(seq_len(n.net), function(j) {
    ...
    # advance to the j'th substream:
    # we use an independent substream of random numbers for each value of the index j
    for(i in seq_len(j))
...
t2b <- system.time({
  ...
  est2b <- sfSapply(seq_len(n.net), function(j) {
    rstream.packed(rngstream) <- FALSE
    for(i in seq_len(j))
      rstream.nextsubstream(rngstream)
